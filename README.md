...
> ⚠️ **注意**：MLI 光场视频的数据格式目前为暂定方案。场景目录下的图像文件（排除 `{u}_{v}` 格式的 SAI 文件）按文件名排序后，以位置索引对应各帧。
